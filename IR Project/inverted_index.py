...
class InvertedIndex:  
  def __init__(self, docs={}):
    ...
    docs_views = {}
    docs_views_sorted_flag = False

    for doc_id, tokens in docs.items():
      self.add_doc(doc_id, tokens)

  @staticmethod
  def page_update_views(doc_id, views):
    InvertedIndex.docs_views[doc_id] = views
  ...
  @staticmethod
  def pages_Ranked_Views(N=3):
    return list(InvertedIndex.docs_views.items).sort(key=lambda x: x[1])[:N]
